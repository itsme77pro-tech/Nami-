...
WATCHLIST = [
    ...
    "GBP/USD",           # Forex - major pair  
    "USD/JPY",           # Forex - major pair
    "USD/CHF",           # Forex - major pair
    "BTC/USDT:BINANCE",  # Crypto - Binance USDT pair
    "ETH/USDT:BINANCE"   # Crypto - Binance USDT pair
]
...
class DataFetcher:
    ...
    def _process_data(self, values: List[dict]) -> pd.DataFrame:
        ...
        # Handle volume (may be missing for forex)
...
class TechnicalAnalyzer:
    ...
    def breakout(df: pd.DataFrame, lookback: int = 20) -> Optional[str]:
        ...
        # Require close beyond range, not just touch
    ...
    def bull_flag(df: pd.DataFrame) -> Optional[str]:
        ...
        # Impulse move (candles 15-10 ago)
        impulse = df["close"].iloc[-15:-10]
        # Consolidation (last 5 candles)
    ...
    def double_top_bottom(df: pd.DataFrame, lookback: int = 10) -> Optional[str]:
        """Detect potential double top/bottom patterns."""
    ...
    def fair_value_gap(df: pd.DataFrame) -> Optional[str]:
        ...
        # Calculate candle 2 body and range
...
class SignalScorer:
    ...
    def calculate_score(
        ...
        """
        Calculate signal score based on multiple confluences.
        ...
        # Trend alignment with 1h (higher timeframe)
...
class NamiBot:
    ...
    async def run(self):
        ...
        while True:
            try:
                async with DataFetcher(TWELVEDATA_KEY) as fetcher:
