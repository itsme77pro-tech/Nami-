...
class WhaleDetectionEngine:
    ...
    def _detect_anomaly(self, symbol, df):
        ...
        if vol_ratio >= WHALE_VOLUME_MULTIPLIER and price_change >= WHALE_PRICE_IMPACT_PCT:
            ...
            if vol_ratio >= 5 and price_change >= 1.0: 
            ...
            return WhaleActivity(
                ...
                direction=direction,
                confidence=min(95, int(50 + vol_ratio * 5 + price_change * 10))
            )
        return None
...
<b>🔥 Portfolio Heat</b>
├ Exposure: <code>{metrics.active_exposure:.2f}R</code>
├ Capacity Left: <code>{metrics.available_capacity:.2f}R</code>
└ Heat Score: <code>{metrics.portfolio_heat_score}%</code>

<b>📉 Drawdown Status</b>
├ Status: <b>{metrics.drawdown_status.upper()}</b>

<b>📊 Active Trades</b>
{chr(10).join(
f"• {t['symbol']} {t['direction']} | PnL: {t['unrealized_pnl']:+.2f}R"
for t in active_trades
) if active_trades else "No active trades"}

⚙️ Auto Risk Monitoring Enabled
"""
    ...
    def risk_dashboard(metrics, active_trades):
        ...
        Args:
            metrics: Object with attributes: portfolio_heat_score, active_exposure, 
                     available_capacity, drawdown_status
...
<b>📊 Portfolio Heat Score</b>
└ {heat_emoji} <code>{heat_score}%</code> {"⚠️ HIGH RISK" if heat_score >= 80 else "✅ HEALTHY" if heat_score <= 50 else "⚡ ELEVATED"}

<b>💰 Exposure Metrics</b>
├ Active Exposure: <code>{getattr(metrics, 'active_exposure', 0):.2f}R</code>
└ Remaining Capacity: <code>{getattr(metrics, 'available_capacity', 0):.2f}R</code>
...
"""
    ...
    # ================= NEW METHOD 2: DAILY PERFORMANCE REPORT =================
    ...
    def can_add_trade(self, new_signal, open_trades):
        ...
        for trade in open_trades:
            ...
            if abs(corr) > 0.7:
                ...
                if corr_exposure + new_signal.position_size_r > MAX_CORRELATION_EXPOSURE: 
...
class ExecutionEngine:
    ...
    def update_trailing_stop(self, trade, current_price):
        """
        FIXED: Complete method with proper syntax
        ...
        if not trade.trailing_active:
            ...
            if profit >= risk * TRAILING_ACTIVATION_R:
                ...
                # Set initial trailing level at breakeven + small buffer
                if trade.signal.direction == "BUY":
...
print("Part 3B loaded: Event Triggers, Risk Manager, Execution Engine (FIXED)")
"""
