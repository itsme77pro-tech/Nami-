...
class TechnicalAnalyzer:
    ...
    def ema_trend(df: pd.DataFrame, fast: int = 20, slow: int = 50) -> str:
        """EMA trend - returns BUY, SELL, or NEUTRAL."""
        if len(df) < slow:
            return "NEUTRAL"
...
class SignalScorer:
    ...
    def calculate_score(self, df_15m: pd.DataFrame, df_1h: pd.DataFrame) -> Tuple[int, str]:
        """
        Calculate signal score using ALL strategies.
        Returns: (score, direction)
        """
        # Get primary direction from EMA trend
        direction = self.analyzer.ema_trend(df_15m)
        
        # If EMA trend is neutral, we can't determine direction
        if direction == "NEUTRAL":
            logger.debug("EMA trend neutral, skipping")
            return 0, "NEUTRAL"
...
class RiskManager:
    ...
    def calculate_dynamic_levels(
        ...
        logger.info(f"Risk levels: Entry={entry:.5f}, SL={sl:.5f}, TP={tp:.5f}, ATR={atr:.5f}, RR={rr:.2f}")
...
class TradeTracker:
    ...
    async def evaluate_trade(self, trade: Trade, df: pd.DataFrame) -> bool:
        """
        Evaluate trade against new candle data.
...
class NamiEngine:
    ...
    async def scan_symbol(self, symbol: str) -> Optional[Signal]:
        ...
        if len(df_15m) < 50 or len(df_1h) < 50:
            logger.warning(f"Insufficient data for {symbol}: 15m={len(df_15m)}, 1h={len(df_1h)}")
            return None
        
        # Calculate score and direction
        score, direction = self.scorer.calculate_score(df_15m, df_1h)
        
        logger.info(f"{symbol}: Score={score}, Direction={direction}")
        
        if direction == "NEUTRAL":
            logger.info(f"{symbol}: Neutral direction, skipping")
            return None
        ...
        logger.info(f"{symbol}: Signal created - {direction} @ {entry:.5f}, Score {score}")
