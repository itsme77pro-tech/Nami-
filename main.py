...
SCAN_INTERVAL = 900
MIN_SCORE = 3 if PERSONALITY == "nami" else 6
MAX_RETRIES = 3
RETRY_DELAY = 5
...
<b>Score:</b> {signal.score}/{12 if PERSONALITY == 'nami' else 13} | <b>Confidence:</b> {confidence_text}
...
# ================= TECHNICAL ANALYZER (15 STRATEGIES) =================
...
class TechnicalAnalyzer:
    ...
    def session_liquidity_sweep_engine(df: pd.DataFrame, session_lookback: int = 24) -> Optional[str]:
        """
        Session Liquidity Sweep Engine (SLSE)
        Detects session-based liquidity sweeps with institutional order flow.
        """
        if len(df) < session_lookback + 5:
            return None
        
        # Define session boundaries (assuming 24 candles = 6 hours for 15min)
        ...
        # Find where liquidity was swept
        recent_candles = df.iloc[-5:]
        
        # Bullish SLSE: Swept session low with strong reversal
        for i in range(len(recent_candles) - 1, -1, -1):
            candle = recent_candles.iloc[i]
            
            # Check if we swept below session low
            if candle['low'] < session_low * 1.001:
                # Look for reversal confirmation in next candles
                if i < len(recent_candles) - 1:
                    next_candles = recent_candles.iloc[i+1:]
                    for j, rev_candle in next_candles.iterrows():
                        # Strong bullish reversal
                        if (rev_candle['close'] > rev_candle['open'] and 
                            rev_candle['close'] > session_low and
                            (rev_candle['close'] - rev_candle['open']) / (rev_candle['high'] - rev_candle['low']) > 0.6):
                            return "BUY"
        
        # Bearish SLSE: Swept session high with strong reversal
        for i in range(len(recent_candles) - 1, -1, -1):
            candle = recent_candles.iloc[i]
            ...
            if candle['high'] > session_high * 0.999:
                if i < len(recent_candles) - 1:
                    ...
                    for j, rev_candle in next_candles.iterrows():
                        ...
                        if (rev_candle['close'] < rev_candle['open'] and 
                            rev_candle['close'] < session_high and
                            (rev_candle['open'] - rev_candle['close']) / (rev_candle['high'] - rev_candle['low']) > 0.6):
                            return "SELL"
    ...
    def daily_liquidity_cycle_model(df: pd.DataFrame, df_1h: Optional[pd.DataFrame] = None) -> Optional[str]:
        """
        Daily Liquidity Cycle Model (DLCM)
        Identifies daily liquidity cycles: Accumulation → Manipulation → Distribution.
        """
        if len(df) < 48:  # Need at least 12 hours of data
            return None
        ...
        # Manipulation detection: Price at extreme with volume divergence
        # Distribution: Price near highs with high volume
        if position_in_range > 0.85 and recent_volume > avg_volume * 1.2:
            # Look for distribution signs (bearish candles at highs)
            recent_candles = df.tail(3)
            bearish_count = sum(1 for _, c in recent_candles.iterrows() if c['close'] < c['open'])
            if bearish_count >= 2:
                return "SELL"
        
        # Accumulation: Price near lows with high volume
        ...
        # Mid-range manipulation: False breakout and reversal
...
class SignalScorer:
    ...
    def calculate_score(self, df_15m: pd.DataFrame, df_1h: pd.DataFrame, df_4h: Optional[pd.DataFrame] = None) -> Tuple[int, str, Optional[str], float, List[str]]:
        """
        Calculate score using all 15 strategies.
        Returns: (score, direction, primary_strategy, adx, all_triggered_strategies)
        """
        direction = None
        score = 0
        ...
        if flag == direction:
            score += 1
            triggered_strategies.append("Bull/Bear Flag")
            if primary_strategy is None:
        ...
        if dt == direction:
            score += 1
            triggered_strategies.append("Double Top/Bottom")
            if primary_strategy is None:
        ...
        if hs == direction:
            score += 1
            triggered_strategies.append("Head & Shoulders")
            if primary_strategy is None:
        ...
        if triangle == direction:
            score += 1
            triggered_strategies.append("Triangle")
            if primary_strategy is None:
        ...
        if fvg == direction:
            score += 1
            triggered_strategies.append("Fair Value Gap")
            if primary_strategy is None:
        ...
        if self.analyzer.volume_spike(df_15m):
            score += 1
            triggered_strategies.append("Volume Spike")
        
        # 10. SME Pattern
        sme = self.analyzer.sme_pattern(df_15m)
        if sme == direction:
            score += 1
            triggered_strategies.append("SME Pattern")
            if primary_strategy is None:
        ...
        if retest == direction:
            score += 1
            triggered_strategies.append("Breakout Retest")
            if primary_strategy is None:
                primary_strategy = "Breakout Retest"
        
        # 12. VWAP Mean Reversion
        vwap = self.analyzer.vwap_mean_reversion(df_15m)
        if vwap == direction:
            score += 1
            triggered_strategies.append("VWAP Reversion")
            if primary_strategy is None:
        ...
        if sweep == direction:
            score += 1
            triggered_strategies.append("Liquidity Sweep")
            if primary_strategy is None:
        ...
        return score, direction or "NEUTRAL", primary_strategy, adx, triggered_strategies
...
class NamiEngine:
    ...
    async def scan_symbol(self, symbol: str) -> Optional[Signal]:
        ...
        score, direction, primary_strategy, adx, triggered_strategies = self.scorer.calculate_score(df_15m, df_1h, df_4h)
    ...
    async def run_scanner(self):
        ...
        strategy_text = (
            ...
            "• <b>SLSE</b> (Session Liquidity Sweep Engine)\n"
            "• <b>DLCM</b> (Daily Liquidity Cycle Model)"
        ) if PERSONALITY == "nami" else (
            "🔥 <b>SNIPER MODE — 15 Strategies + 4H Filter</b> 🔥\n"
            "• All 15 strategies active including SLSE & DLCM\n"
            "• Triple timeframe confirmation\n"
            "• 80-90% confidence only\n"
        ...
        await self.telegram.send_message(
            ...
            f"<b>Min Score:</b> {MIN_SCORE}/{12 if PERSONALITY == 'nami' else 13} | "
                    ...
                    try:
                        ...
                        if signal:
                            ...
                            logger.info(f"✅ {'SNIPER SHOT' if PERSONALITY == 'sogeking' else 'SIGNAL'}: "
                                      ...
                                      f"Primary:{signal.strategy_type} "
                                      f"Strategies:{len(signal.triggered_strategies)} "
