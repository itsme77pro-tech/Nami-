...
"""
Part 3: Whale Detection, Message Templates, Event Triggers, Risk Management
"""
    ...
    def update_trailing_stop(self, trade, current_price):
        if not trade.trailing_active:
            ...
            if profit >= risk * TRAILING_ACTIVATION_R:
                ...
                return trade.signal.entry_price + (risk * 0.1) if trade.signal.direction == "BUY" else trade.signal.entry_price - (risk * 0.1)
        else:
            risk = abs(trade.signal.entry_price - trade.signal.stop_loss)
            if trade.signal.direction == "BUY":
                new_lvl = current_price - (risk * 0.5)
                return max(new_lvl, trade.trailing_level or 0)
            else:
                new_lvl = current_price + (risk * 0.5)
                return min(new_lvl, trade.trailing_level or float('inf'))
        return None

print("Part 3 loaded: Whale Detection, Message Templates, Event Triggers, Risk, Execution")
"""
