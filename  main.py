...
WHALE_PRICE_IMPACT_PCT = 0.5  # Minimum price impact percentage
PROGRESS_BAR_LENGTH = 10
MAX_RECENT_ACTIVITIES = 1000  # Prevent memory leak

# ================= DATA STRUCTURES =================

WhaleActivity = namedtuple('WhaleActivity', [
    'symbol', 'timestamp', 'activity_type', 'volume_anomaly', 
    'price_impact', 'direction', 'confidence', 'momentum_score'
])
...
class WhaleDetectionEngine:
    def __init__(self):
        # Use bounded deques to prevent memory leaks
        self.volume_baselines = {}
        self.price_history = {}
        self.atr_history = {}
        # Bounded list to prevent memory leak [^1^]
        self.recent_activities = deque(maxlen=MAX_RECENT_ACTIVITIES)
        self._initialized_symbols = set()
    
    def _get_symbol_data(self, symbol):
        """Lazy initialization to avoid creating keys on __init__"""
        if symbol not in self._initialized_symbols:
            self.volume_baselines[symbol] = deque(maxlen=100)
            self.price_history[symbol] = deque(maxlen=50)
            self.atr_history[symbol] = deque(maxlen=20)
            self._initialized_symbols.add(symbol)
        return (self.volume_baselines[symbol], 
    ...
    def update(self, symbol, df):
        ...
        # Get or create bounded deques for this symbol
        vol_baselines, price_hist, atr_hist = self._get_symbol_data(symbol)
        
        vol_baselines.append(df['volume'].iloc[-1])
        price_hist.append({
            'price': df['close'].iloc[-1], 
    ...
    def _calculate_atr(self, df, period=14):
        ...
        # Use Wilder's smoothing (RMA) instead of simple moving average
        atr = pd.Series(tr).ewm(alpha=1/period, min_periods=period).mean().iloc[-1]
        return atr
    
    def _detect_anomaly(self, symbol, df):
        vol_baselines, price_hist, atr_hist = self._get_symbol_data(symbol)
        
        if len(vol_baselines) < 20: 
            return None
        
        curr_vol = df['volume'].iloc[-1]
        avg_vol = np.mean(list(vol_baselines)[:-5])
        if avg_vol == 0: 
            return None
        ...
        if vol_ratio >= 3.0 and price_change >= WHALE_PRICE_IMPACT_PCT:
            ...
            atr_valid = self._check_atr_displacement(symbol, df, price_change, atr_hist)
            if not atr_valid:
                return None
    ...
    def _check_atr_displacement(self, symbol, df, price_change, atr_hist):
        ...
        avg_atr = np.mean(list(atr_hist)[-5:])
    ...
    def _detect_multi_candle_momentum(self, df, direction, lookback=5):
        ...
        # Volume progression check - fix pandas truth value ambiguity
        volumes = recent_candles['volume'].values
        vol_trend = all(volumes[i] <= volumes[i+1] for i in range(len(volumes)-1))
        
        # Price velocity check
        price_velocities = []
        closes = recent_candles['close'].values
        for i in range(1, len(recent_candles)):
            vel = abs(closes[i] - closes[i-1]) / closes[i-1] * 100
...
class MessageTemplateLibrary:
    ...
    def _escape_html(text):
        """Escape HTML special characters for Telegram [^7^]"""
        if text is None:
            return "N/A"
    ...
    def session_start_alert(session_name, market_context=None):
        ...
        focus_pairs = market_ctx.get('focus_pairs', ['Major pairs'])
        pairs_list = "\n".join(f"  • {MessageTemplateLibrary._escape_html(pair)}" for pair in focus_pairs)
        
        return f"""
{session_emoji} <b>TRADING SESSION STARTED — {MessageTemplateLibrary._escape_html(session_name)}</b> {session_emoji}

<b>🌍 Market Environment</b>
├ Volatility: {vol_indicator} {MessageTemplateLibrary._escape_html(volatility)}
├ Trend Bias: {trend_indicator} {MessageTemplateLibrary._escape_html(trend)}
...
"""
    ...
    def news_signal_alert(event, sentiment, impact_level='HIGH', details=None):
        ...
        currency_pairs = details.get('currency_pairs', ['Check calendar'])
        pairs_list = "\n".join(f"  • {MessageTemplateLibrary._escape_html(pair)}" for pair in currency_pairs)
        
        return f"""
📰 <b>NEWS EVENT SIGNAL</b>

<b>📊 Event Details</b>
├ Event: <b>{MessageTemplateLibrary._escape_html(event)}</b>
├ Impact: {impact_emojis.get(impact_level, impact_level)}
├ Sentiment: {sentiment_emojis.get(sentiment, sentiment)} <b>{MessageTemplateLibrary._escape_html(sentiment)}</b>
└ Time: <code>{datetime.now(pytz.UTC).strftime('%H:%M')} UTC</code>

<b>📈 Data Release</b>
├ Forecast: <code>{MessageTemplateLibrary._escape_html(forecast)}</code>
├ Actual: <code>{MessageTemplateLibrary._escape_html(actual)}</code>{deviation}
└ Previous: <code>{MessageTemplateLibrary._escape_html(previous)}</code>
...
"""
    ...
    def smc_signal_alert(symbol, direction, structure_type, details=None):
        ...
        confidence_score = details.get('confidence_score', 3)
        
        return f"""
🎓 <b>SMART MONEY SIGNAL</b>

<b>📊 Market Structure</b>
├ Pair: <b>{MessageTemplateLibrary._escape_html(symbol)}</b>
├ Direction: {direction_emojis.get(direction, direction)}
├ Structure: {structure_emojis.get(structure_type, structure_type)}
└ Timeframe: <code>{MessageTemplateLibrary._escape_html(timeframe)}</code>

<b>🎯 Trade Setup</b>
├ Entry Zone: <code>{MessageTemplateLibrary._escape_html(entry_zone)}</code>
├ Stop Loss: <code>{MessageTemplateLibrary._escape_html(stop_loss)}</code>
├ Take Profit: <code>{MessageTemplateLibrary._escape_html(take_profit)}</code>
└ Price Position: {pos_emoji}

<b>🏛️ Institutional Context</b>
├ Liquidity Level: {MessageTemplateLibrary._escape_html(details.get('liquidity_level', 'N/A'))}
├ Order Flow: {MessageTemplateLibrary._escape_html(details.get('order_flow', 'N/A'))}
├ HTF Bias: {MessageTemplateLibrary._escape_html(details.get('htf_bias', 'N/A'))}
└ Session: {MessageTemplateLibrary._escape_html(details.get('session', 'N/A'))}

<b>⚠️ Risk Management</b>
├ Position Size: <code>{MessageTemplateLibrary._escape_html(details.get('position_size', 'Standard'))}R</code>
├ Confidence: {"⭐" * confidence_score}
└ Invalidation: <code>{MessageTemplateLibrary._escape_html(details.get('invalidation_level', 'Structure break'))}</code>

<i>🧠 Wait for LTF confirmation before entry</i>
<code>ID: {MessageTemplateLibrary._escape_html(details.get('setup_id', 'SMC_' + datetime.now(pytz.UTC).strftime('%H%M')))}</code>
"""
    ...
    def trade_entry_alert(signal, execution_details=None, market_context=None):
        ...
        confluence_list = "\n".join(f"  ✅ {MessageTemplateLibrary._escape_html(factor)}" for factor in confluence_factors[:4])
        
        return f"""
{entry_emoji} <b>POSITION OPENED — {MessageTemplateLibrary._escape_html(signal.symbol)}</b> {entry_emoji}
...
└ Strategy: <b>{MessageTemplateLibrary._escape_html(signal.primary_strategy)}</b>
...
<code>ID: {MessageTemplateLibrary._escape_html(signal.id)}</code>
"""
    ...
    def trade_exit_alert(trade, exit_details):
        ...
        return f"""
{pnl_emoji} <b>POSITION CLOSED — {MessageTemplateLibrary._escape_html(signal.symbol)}</b> {pnl_emoji}
...
├ Strategy: <b>{MessageTemplateLibrary._escape_html(signal.primary_strategy)}</b>
└ Market Regime: <code>{MessageTemplateLibrary._escape_html(signal.regime.primary.name)}</code>

<i>⏱️ Closed at {trade.exit_time.strftime('%H:%M:%S')} UTC</i>
<code>ID: {MessageTemplateLibrary._escape_html(signal.id[:8])}</code>
"""
    ...
    def partial_profit_alert(trade, partial_details):
        ...
        current_price = partial_details.get('current_price', 'N/A')
        
        return f"""
💰 <b>PARTIAL PROFIT SECURED — {MessageTemplateLibrary._escape_html(signal.symbol)}</b> 💰
...
"""
    ...
    def stop_loss_update_alert(trade, old_sl, new_sl, reason):
        ...
        """
        sl_improvement = abs(new_sl - trade.signal.entry_price) / abs(old_sl - trade.signal.entry_price) if old_sl != trade.signal.entry_price else 0
        
        return f"""
🛡️ <b>STOP LOSS UPDATED — {MessageTemplateLibrary._escape_html(trade.signal.symbol)}</b> 🛡️

<b>📊 Adjustment Details</b>
├ Reason: <b>{MessageTemplateLibrary._escape_html(reason)}</b>
...
<code>ID: {MessageTemplateLibrary._escape_html(trade.signal.id[:8])}</code>
"""
    ...
    def risk_dashboard(metrics, active_trades):
        ...
        if display_trades:
            for trade in display_trades:
                ...
                trades_list.append(f"  {dir_emoji} <code>{MessageTemplateLibrary._escape_html(symbol)}</code> {direction} | {pnl_emoji} {pnl:+.2f}R")
...
├ Correlation Risk: <code>{MessageTemplateLibrary._escape_html(correlation_risk)}</code>
└ Session Risk: <code>{MessageTemplateLibrary._escape_html(session_risk)}</code>
...
"""
    ...
    def pre_signal_alert(signal, time_to_signal):
        ...
        confluence_preview = MessageTemplateLibrary._format_confluence_preview(signal.confluence_factors)
        
        return f"""
⏰ <b>SETUP FORMING — {MessageTemplateLibrary._escape_html(signal.symbol)}</b>
...
"""
    ...
    def _format_confluence_preview(factors):
        ...
        preview = [f"  {i}. {MessageTemplateLibrary._escape_html(factor)} ⚡" for i, factor in enumerate(factors[:3], 1)]
        if len(factors) > 3: 
    ...
    def signal_alert(signal, market_context):
        ...
        confluence_detailed = MessageTemplateLibrary._format_confluence_detailed(signal.confluence_factors)
        
        return f"""
🎯 <b>SIGNAL CONFIRMED — {MessageTemplateLibrary._escape_html(signal.symbol)}</b>
...
└ Reward:  {reward_bar} ${reward_amount:.2f}"""
    ...
    def _generate_progress_bar(current, total, reverse=False):
        ...
        if reverse:
            filled = PROGRESS_BAR_LENGTH - filled
        bar = "█" * filled + "░" * (PROGRESS_BAR_LENGTH - filled)
        return f"<b>[{bar}]</b>"
    ...
    def _format_confluence_detailed(factors):
        if not factors:
            return "  No confluence factors available"
        lines = []
        weights = [2.0, 1.5, 1.0, 0.8, 0.5]
        for i, factor in enumerate(factors[:5]):
            weight = weights[min(i, 4)]
            bar = "█" * int(weight * 3)
            lines.append(f"  {bar} {MessageTemplateLibrary._escape_html(factor)}")
        return "\n".join(lines)
...
"""
    ...
    def session_open_ceremony(session_type, high_impact_news=None):
        ...
        if high_impact_news:
            news_list = "\n".join(f"  • {MessageTemplateLibrary._escape_html(news)}" for news in high_impact_news)
            news_alert = f"\n⚠️ <b>HIGH IMPACT NEWS (Next 2H):</b>\n{news_list}\n"
        else:
            news_alert = ""
            
        return f"""
{session_emoji} <b>{MessageTemplateLibrary._escape_html(session_type)} SESSION OPEN</b> {session_emoji}
...
<b>🎯 Today's Focus:</b>
  • Major pairs with {MessageTemplateLibrary._escape_html(session_type)} exposure
...
"""
    ...
    def daily_performance_report(trades, portfolio_state):
        ...
        if trades:
            ...
            best_signal = getattr(best_trade, 'signal', None)
            worst_signal = getattr(worst_trade, 'signal', None)
            best_symbol = getattr(best_signal, 'symbol', 'N/A') if best_signal else 'N/A'
            worst_symbol = getattr(worst_signal, 'symbol', 'N/A') if worst_signal else 'N/A'
        else:
...
├ Best Trade: 🏆 <code>{MessageTemplateLibrary._escape_html(best_symbol)} ({best_pnl:+.2f}R)</code>
└ Worst Trade: 💀 <code>{MessageTemplateLibrary._escape_html(worst_symbol)} ({worst_pnl:+.2f}R)</code>
...
class PortfolioRiskManager:
    ...
    def get_risk_metrics(self):
        ...
        # Build top_risks list from current state
        top_risks = []
        if self.state.current_drawdown_pct > self.DRAWDOWN_PROTECTION_PCT:
            top_risks.append(f"High drawdown: {self.state.current_drawdown_pct:.1f}%")
        if self.risk_multiplier < 1.0:
            top_risks.append(f"Risk reduced: {self.risk_multiplier:.0%} capacity")
        if self.state.total_exposure_r > MAX_PORTFOLIO_EXPOSURE * 0.8:
            top_risks.append(f"High exposure: {self.state.total_exposure_r:.1f}R")
        ...
        return RiskMetrics(
            ...
            correlation_risk=50,
            session_risk=30,
            top_risks=top_risks[:3]  # Limit to top 3 risks
        )
...
class IntegratedNotificationSystem:
    ...
    async def send_pre_signal(self, signal, time_to_signal): 
        await self.triggers.trigger_event(AlertType.PRE_SIGNAL, {'signal': self._signal_to_dict(signal), 'time_to_signal': time_to_signal}, priority=2)
    ...
    async def send_liquidity_alert(self, symbol, zones, current_price): 
        await self.triggers.trigger_event(AlertType.LIQUIDITY_ZONE, {'zones': [self._zone_to_dict(z) for z in zones], 'current_price': current_price}, priority=3)
    
    async def send_session_open(self, session_type, high_impact_news=None): 
        await self.triggers.trigger_event(AlertType.SESSION_OPEN, {'session_type': session_type, 'high_impact_news': high_impact_news or []}, priority=3)
    
    # Proxy methods for direct trigger access
    async def trigger_event(self, alert_type, data, priority=2):
        """Proxy method to access triggers directly"""
        await self.triggers.trigger_event(alert_type, data, priority)
    ...
    def _signal_to_dict(self, signal):
        return {
            ...
            'countdown_seconds': getattr(signal, 'countdown_seconds', 0), 
            'setup_quality_score': getattr(signal, 'setup_quality_score', 0), 
            'confluence_factors': getattr(signal, 'confluence_factors', []), 
            'risk_visualization': getattr(signal, 'risk_visualization', {}), 
            'probability_breakdown': getattr(signal, 'probability_breakdown', {})
        }
    
    def _whale_to_dict(self, a): 
        return {
            'symbol': a.symbol, 
            'timestamp': a.timestamp.isoformat(), 
            'activity_type': a.activity_type, 
            'volume_anomaly': a.volume_anomaly, 
            'price_impact': a.price_impact, 
            'direction': a.direction, 
            'confidence': a.confidence
        }
    
    def _zone_to_dict(self, z): 
        return {
            'symbol': z.symbol, 
            ...
            'volume_at_level': z.volume_at_level
        }
    
    def _risk_to_dict(self, m): 
        return {
            'timestamp': m.timestamp.isoformat(), 
            'portfolio_heat_score': m.portfolio_heat_score, 
            'active_exposure': m.active_exposure, 
            'available_capacity': m.available_capacity, 
            'drawdown_status': m.drawdown_status, 
            'correlation_risk': m.correlation_risk, 
            'session_risk': m.session_risk, 
            'top_risks': m.top_risks
        }
...
class SignalPersistence:
    ...
    def save_signal(self, signal):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("""
                ...
                (id, symbol, direction, entry_price, stop_loss, take_profit, timestamp, probability, status, closed_pnl, metadata)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                signal.id, 
                signal.symbol, 
                ...
                signal.probability.final_probability if hasattr(signal.probability, 'final_probability') else signal.probability, 
                "active", 
                0.0, 
                json.dumps(asdict(signal) if hasattr(signal, '__dataclass_fields__') else vars(signal))
            ))
            conn.commit()
    ...
    def update_trade_close(self, trade):
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                "UPDATE signals SET status = ?, closed_pnl = ? WHERE id = ?", 
            ...
            conn.execute("""
                INSERT INTO trades 
                (id, signal_id, entry_executed, exit_price, exit_time, status, outcome, realized_pnl_r)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                f"{trade.signal.id}_exec", 
                trade.signal.id, 
                trade.entry_executed, 
                trade.exit_price, 
                trade.exit_time.isoformat() if trade.exit_time else None, 
                trade.status.value if hasattr(trade.status, 'value') else str(trade.status), 
                trade.outcome.value if hasattr(trade.outcome, 'value') else str(trade.outcome), 
                trade.realized_pnl_r
            ))
            conn.commit()
...
class TradingSessionEngine:
    ...
    def __init__(self):
        # Fixed: Non-overlapping time ranges in UTC
        self.session_ranges = {
            'SYDNEY': (22, 24),    # 22:00 - 00:00 (continues to 07:00 next day handled in logic)
            'TOKYO': (0, 7),       # 00:00 - 07:00 (extended Tokyo session)
            'LONDON': (8, 13),     # 08:00 - 13:00 (before overlap)
            'OVERLAP': (13, 17),   # 13:00 - 17:00 (London + NY overlap)
            'NEW_YORK': (17, 22),  # 17:00 - 22:00 (after overlap)
        }
    ...
    def get_current_session(self):
        ...
        # Sydney session: 22:00 - 07:00 (wraps around midnight)
        if hour >= 22 or hour < 7:
            return 'SYDNEY'
        
        # Tokyo: 7:00 - 8:00 (gap between Sydney and London)
        if 7 <= hour < 8:
            return 'TOKYO'
        ...
        # New York solo session: 17:00 - 22:00
        if 17 <= hour < 22:
            return 'NEW_YORK'
    ...
    def get_next_session_transition(self):
        ...
        # Transition hours in order: 7 (Sydney->Tokyo), 8 (Tokyo->London), 13 (London->Overlap), 17 (Overlap->NY), 22 (NY->Sydney)
        transition_hours = [7, 8, 13, 17, 22]
        
        for transition in transition_hours:
            if hour < transition:
                return transition - hour
        
        # If past 22:00, next transition is 7:00 tomorrow
        return (24 - hour) + 7
    ...
    def check_news_conflict(self, symbol, direction):
        ...
        for event in self.events:
            if event['symbol'] in symbol and event['impact'] == 'high':
                ...
                if 0 < time_to_event < 60:  # Within 60 minutes
...
class SmartMoneyConceptEngine:
    ...
    def _detect_order_blocks(self, df):
        ...
        for i in range(3, len(df)-1):
            # Bullish OB: preceding bearish candle, then bullish engulfing
            if (df['close'].iloc[i] > df['open'].iloc[i] and 
                df['close'].iloc[i-1] < df['open'].iloc[i-1] and
                df['close'].iloc[i] > df['open'].iloc[i-1]):  # Engulfing condition
                obs.append({
                    'type': 'bullish', 
                    'price': df['low'].iloc[i-1], 
                    'strength': abs(df['close'].iloc[i] - df['open'].iloc[i]) / df['close'].iloc[i]
                })
            # Bearish OB: preceding bullish candle, then bearish engulfing
                ...
                obs.append({
                    ...
                    'strength': abs(df['open'].iloc[i] - df['close'].iloc[i]) / df['close'].iloc[i]
                })
    ...
    def _detect_fvg(self, df):
        ...
        for i in range(2, len(df)-1):
            # Bullish FVG: current low > previous high (2 candles back)
            ...
            # Bearish FVG: current high < previous low (2 candles back)
    ...
    def _detect_liquidity_sweeps(self, df, symbol):
        ...
        sweeps = []
        # Check for sweep of recent high (buyside liquidity) followed by rejection
        if (df['high'].iloc[-3] >= recent_high and 
        ...
        # Check for sweep of recent low (sellside liquidity) followed by rejection
        if (df['low'].iloc[-3] <= recent_low and 
            df['close'].iloc[-3] > recent_low and
            df['close'].iloc[-2] > recent_low):
            sweeps.append({
    ...
    def _determine_bias(self, order_blocks, fvg_zones, current_price):
        ...
        # Add FVG bias
        bullish_fvg = sum(1 for fvg in fvg_zones if fvg['type'] == 'bullish' and current_price > fvg['bottom'])
        bearish_fvg = sum(1 for fvg in fvg_zones if fvg['type'] == 'bearish' and current_price < fvg['top'])
        
        bullish_total = bullish_count + bullish_fvg
        bearish_total = bearish_count + bearish_fvg
    ...
    def _calculate_smc_confidence(self, order_blocks, fvg_zones, liquidity_sweeps):
        ...
        score = 0
        if order_blocks: 
            score += min(len(order_blocks) * 10, 30)
        if fvg_zones: 
            score += min(len(fvg_zones) * 10, 20)
        if liquidity_sweeps: 
            score += 25
        return min(score, 60)
...
class BacktestingEngine:
    ...
    async def _simulate_trades(self, symbol, df):
        ...
        # Placeholder - implement actual simulation
        pass
...
class HedgeFundEngine:
    ...
    def get_asset_class(self, symbol): 
        return 'crypto' if any(c in symbol for c in ['BTC', 'ETH', 'SOL', 'BNB', 'XRP', 'ADA']) else 'forex'
    ...
    async def scan_symbol(self, symbol):
        ...
        if not primary or primary.confidence < 65: 
            return None
        ...
        smc_alignment = ((smc_bias == 'bullish' and technical_bias == 'BUY') or 
                        (smc_bias == 'bearish' and technical_bias == 'SELL'))
        ...
        if primary.entry_price: 
            entry = primary.entry_price
        if primary.stop_loss: 
            sl = primary.stop_loss
        else: 
            sl = entry - atr * ATR_MULTIPLIER_SL if primary.direction == "BUY" else entry + atr * ATR_MULTIPLIER_SL
        
        if primary.take_profit: 
            tp = primary.take_profit
        else: 
            tp = entry + atr * ATR_MULTIPLIER_TP if primary.direction == "BUY" else entry - atr * ATR_MULTIPLIER_TP
        ...
        prob_model = ProbabilityModel(
            base_rate=combined_confidence / 100,
            market_condition=combined_confidence / 100,
            volatility_adj=(0.05 if regime.volatility == VolatilityRegime.NORMAL else 
                         -0.05 if regime.volatility == VolatilityRegime.LOW else 0),
            liquidity_boost=nearest_liq.strength if nearest_liq else 0,
            flow_alignment=1.0 if orderflow.displacement else 0.5,
            max_probability=min(0.95, combined_confidence / 100),
            edge=edge,
            kelly_fraction=kelly_frac,
            confidence_interval=(max(0, combined_confidence/100 - 0.15), 
                               min(1, combined_confidence/100 + 0.15))
        )
        ...
        signal = Signal(
            ...
            confluence_strategies=[d.name for d in detections],
            liquidity_zones=[nearest_liq] if nearest_liq else [],
            orderflow=orderflow,
            position_size_r=kelly_frac * 100,
            risk_reward=rr,
            expectancy=(prob_model.final_probability * rr) - ((1 - prob_model.final_probability) * 1)
        )
...
"""
        ...
        ux_signal = UXSignal(
            ...
            countdown_seconds=0,
            setup_quality_score=int(combined_confidence),
        ...
        await self.notification_system.send_signal(ux_signal, {
            'regime': regime.primary.name if hasattr(regime, 'primary') else str(regime),
            'session': session,
            'volatility': regime.volatility.name if hasattr(regime.volatility, 'name') else str(regime.volatility),
            'primary_strategy': primary.name,
            'smc_bias': smc_bias,
            'news_status': 'clear'
        })
    ...
    async def evaluate_trades(self):
        for trade in list(self.active_trades):
            ...
            try:
                ...
                if trade.signal.direction == "BUY": 
                    trade.unrealized_pnl_r = ((current_price - trade.signal.entry_price) / 
                                             abs(trade.signal.entry_price - trade.signal.stop_loss))
                else: 
                    trade.unrealized_pnl_r = ((trade.signal.entry_price - current_price) / 
                                             abs(trade.signal.entry_price - trade.signal.stop_loss))
                ...
                # Check partial TP
                if (trade.status == TradeStatus.PENDING and 
                    self.execution.should_trigger_partial(trade, current_price)):
                    ...
                    await self.notification_system.trigger_event(
                        AlertType.SIGNAL, 
                        {'message': f"Partial TP hit for {trade.signal.symbol}: +{trade.realized_pnl_r:.2f}R secured"}, 
                        priority=2
                    )
                ...
                if trade.signal.direction == "BUY":
                    if current_price <= trade.signal.stop_loss: 
                        exit_triggered, exit_price, exit_reason = True, trade.signal.stop_loss, TradeOutcome.SL_HIT
                    elif (trade.trailing_active and trade.trailing_level and 
                          current_price <= trade.trailing_level): 
                        exit_triggered, exit_price, exit_reason = True, trade.trailing_level, TradeOutcome.TRAILING_STOP
                    elif current_price >= trade.signal.take_profit: 
                        exit_triggered, exit_price, exit_reason = True, trade.signal.take_profit, TradeOutcome.TP_HIT
                else:
                    if current_price >= trade.signal.stop_loss: 
                        exit_triggered, exit_price, exit_reason = True, trade.signal.stop_loss, TradeOutcome.SL_HIT
                    elif (trade.trailing_active and trade.trailing_level and 
                ...
                if exit_triggered:
                    ...
                    # Calculate final PnL
                    if trade.signal.direction == "BUY": 
                        full_pnl = ((executed_exit - trade.signal.entry_price) / 
                                   abs(trade.signal.entry_price - trade.signal.stop_loss))
                    else: 
                        full_pnl = ((trade.signal.entry_price - executed_exit) / 
                                   abs(trade.signal.entry_price - trade.signal.stop_loss))
                    ...
                    # Fixed: Proper trade status determination
                    if exit_reason == TradeOutcome.TP_HIT:
                        ...
                        if trade.realized_pnl_r > 0.5:
                            trade.status = TradeStatus.WIN
                        elif trade.realized_pnl_r > 0:
                    ...
                    if trade.status == TradeStatus.LOSS:
                        ...
                        await self.notification_system.trigger_event(
                            AlertType.SIGNAL, 
                            {
                                'message': f"Trade closed: {trade.signal.symbol} {trade.status.value.upper()} | P&L: {trade.realized_pnl_r:+.2f}R"
                            }, 
                            priority=2
                        )
                
                # Time-based expiry check (96 candles = 24 hours on 15m)
                ...
                if trade.candles_evaluated > 96 and abs(trade.unrealized_pnl_r) < 0.5:
                    ...
                    await self.notification_system.trigger_event(
                        ...
                        {
                            'symbol': trade.signal.symbol, 
                            'reason': 'Time expiry - position held too long without meaningful move', 
                            'entry': trade.signal.entry_price, 
                            'current': current_price, 
                            'loss': trade.unrealized_pnl_r, 
                            'trade_id': trade.signal.id
                        }, 
                        priority=3
                    )
...
{news_text}"""
        ...
        for symbol in WATCHLIST:
            try:
                ...
                if result: 
                    self.persistence.save_signal(result)
                await asyncio.sleep(1)
            except Exception as e: 
        ...
        try:
            ...
            try:
                await engine.telegram.send_message(
                    ...
                    f"Engine stopped due to error: {str(e)[:100]}\n"
